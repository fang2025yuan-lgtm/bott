...
def generate_qr_data(event_id: int, user_telegram_id: int, security_hash: str):
    ...
    return f"https://t.me/{BOT_USERNAME}?start=chk_E{event_id}_U{user_telegram_id}_S{security_hash}"
...
async def generate_ticket_image(
    ...
    Returns:
        BytesIO: Rasm bytes formatida
    """
    # Rasm o'lchamlari
    width, height = 800, 450
    
    # Fon ranglar (gradient effekt uchun)
    bg_color = (15, 23, 42)  # To'q ko'k
    accent_color = (56, 189, 248)  # Och ko'k
    ...
    try:
        # Shriftlarni yuklash (default font ishlatamiz)
    ...
    # QR kodni asosiy rasmga joylashtirish
    ...
    draw.text((width // 2 - 100, height - 35), f"© {datetime.now().year} UFQ Events", font=small_font, fill=text_color)
