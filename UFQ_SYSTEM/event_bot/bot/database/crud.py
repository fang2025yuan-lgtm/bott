...
async def mark_attendance(reg_id: int, status: RegStatus):
    """Mark attendance. Updates registration via SQLAlchemy and points via raw query."""
    async with AsyncSessionLocal() as session:
        ...
        if status == RegStatus.ATTENDED and reg.status != RegStatus.ATTENDED:
            point_change = event.attendance_points
    ...
    if point_change != 0:
        ...
        user_data = await get_user_by_tg_id(user_row['telegram_id'])
        if user_data:
            await update_user_status_if_needed(user_data['telegram_id'])
...
async def verify_and_checkin(security_hash: str, event_id: int, scanner_tg_id: int):
    ...
    if old_status != new_status:
        ...
        status_change_msg = f"\n Tabriklaymiz! Sizning statusingiz o'zgardi: {old_name} -> {new_name}"

    success_msg = (
        f"Muvaffaqiyatli!\n\n"
        f"{user_data['full_name']}\n"
        f"{event.title}\n"
        f"+{event.attendance_points} ball qo'shildi (Jami: {new_points}){status_change_msg}"
    )

    return True, success_msg, user_data['full_name']
